...
from postprocessing.training.ridge_runner import fit_ridge
...
def main():
    ...
    print(f"  Targets: {len(TARGETS)}, leads: {LEADS}, alphas: {ALPHAS}")
    total = len(TARGETS) * len(LEADS) * len(stations) * len(ALPHAS)
    ...
    print(f"=== Ridge multilead sweep complete in {total_elapsed:.1f}s ({total_elapsed/60:.1f} min) ===")
